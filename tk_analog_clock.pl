...
use vars qw( %clock %obj $_debug $_test $_verbose  $_pi $mode_24h %calculated );
...
# print Data::Dumper->Dump( [ \%max, \$radius, \$i, \%inner, \%outer, ], [ qw( *max *radius *i *inner *outer ) ] ), qq{\n};
...

    {
        ...
        $inner{hour}{radius} = int( $radius * ( 5.0 / 6.0 ) );
        ...
        ( $inner{hour}{radius}, $inner{second}{radius}, ) = ( $inner{second}{radius}, $inner{hour}{radius}, );
        ( $outer{hour}{radius}, $outer{second}{radius}, ) = ( $outer{second}{radius}, $outer{hour}{radius}, );
